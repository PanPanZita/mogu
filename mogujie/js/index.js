(function(){
	$(function(){
		...
		$('.haudong').get('0').onscroll = function () {
			// console.log(1);
			// var event = evt || window.which;
			// var scrollTop = $('.haudong')[0].documentElement.scrollTop || $('.haudong')[0].body.scrollTop;
		...
		/*$('.haudong').scroll(function(evt){
			var event = evt || window.which;
			var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
			if(scrollTop > 10){
				$('header').css({position:'fixed',left:'0',right:'0',zIndex:'2'});
				download.css({display:'none'});
			}else{
				$('header').css({position:'relative'});
				download.css({display:'block'});
